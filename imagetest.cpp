    ...
    , ui(new Ui::ImageTest)
{
    ...
    connect(ui->pushButton,&QPushButton::clicked,this,&ImageTest::controlWorker);
    ...
    //利用定时器不断进行人脸识别请求
    netTimer = new QTimer();
    connect(netTimer,&QTimer::timeout,this,&ImageTest::controlWorker);

    //进行界面布局
    this->resize(1200,700);
    finder->setMinimumSize(300,250);
    ...
    //query.addQueryItem("client_id","vsRmajo8BxycoSEDk4bOVkcG");
    //query.addQueryItem("client_secret","xb6a8cijU2L46rrzd1x4Tf4UQ5oK5HDu");

    //query.addQueryItem("client_id","oQXBR1aW4BrlvqrvMpCAkyCd");
    //query.addQueryItem("client_secret","BxjQcEpqiKDuUR19e1vv3F4CFTBQgPpj");

    //手势识别
    //query.addQueryItem("client_id","te1yteFPArHjxpGqcDeTkVlD");
    //query.addQueryItem("client_secret","9bPtNdzciMqGV9BW0ONleERhqu1vejIs");
...
void ImageTest::showCamera(int id,QImage preview)
{
    Q_UNUSED(id);
    img = preview;
    //绘制人脸框
...
void ImageTest::controlWorker()
{
    Worker* worker = new Worker;
    QThreadPool::globalInstance()->start(worker);

    connect(this,&ImageTest::operate,worker,&Worker::setImage);
...
void ImageTest::beginFaceDetect(QByteArray postData)
{
    //组装图像识别请求
